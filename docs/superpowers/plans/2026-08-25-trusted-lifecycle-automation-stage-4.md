...
- [ ] Add the six Task 2 nodes. Evolve both exact route-count owners from `184` to `187` and lifecycle route count from `14` to `17` only after the three-route RED is admitted.
...
- [ ] Run exact Task 2 RED/GREEN, route inventory `187`, and provider-free read/write boundary tests. Commit tests before product code.
