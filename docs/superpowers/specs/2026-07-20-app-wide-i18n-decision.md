...
> LIVE COMPLETE AT PRODUCT `7b9c13b2`, FAST-FORWARD MERGED THROUGH
> `2997c926`; THE PRODUCTION SELECTOR IS LIVE AND STORED `ui_locale` REMAINS
> ABSENT UNTIL THE USER'S FIRST INTENTIONAL SELECTION.**
...
> public selector remained withheld through those migration units. I18N-3
> Explore passed independent
...
> exact `+12/-4`, `95/1056`, and focused `13/148` evidence. Repaired
> independent review returned GREEN, the user approved integration, and
> `master` fast-forwarded through evidence tip `2997c926`. Fresh merged-tree
> gates are green. The production selector was located semantically at
> `zh-Hant` without changing it, so `profile_settings.ui_locale` remains absent
> and the first authority write still belongs to the user.
...
| [I18N-6 Release](2026-07-25-i18n-6-release-design.md) | full coverage/audit, both-locale visual matrix, selector in Settings PageHeader, formatter freeze, docs/Design Kit release sync | LIVE — product `7b9c13b2`; merged/evidence `2997c926` | visible in Settings; production preference remains absent until the user's first selection |
...
6. I18N-6 passed repaired independent review, explicit approval, fast-forward
   integration through `2997c926`, and merged-tree release gates. The public
   selector is LIVE in Settings. Its production authority row remains absent
   because closeout deliberately did not change the selection; the first write
   remains a user action. External Design Kit locale synchronization is the
   post-LIVE follow-up.
