...
## Task 4 - Independent Inventory Admission

Task 4 used two fresh detached worktrees: frozen candidate source
`4c6b8d44ce2e768e95b822b11f618cc40f4bb9f0` and exact Task 3 authority tip
`7afbbf837b79b184825b3e04818dedd45940e4ba`. Only the two pinned collection
helpers were copied. The reviewer scanner, static/dynamic route checker,
projector, validator, encrypted-boundary checker, safety checker, and leak
scanner were written independently from the plan's literal contracts; they did
not copy or import the Task 1-3 executor tools. Full executor, reviewer, and
pinned-helper hashes are in the admission packet.

### Independent recapture and byte comparisons

Fresh collect-only/list recapture reproduced backend
`4394/b0285ee3a3d124c4bbe380ad0dea022ef09fa46b52b6a14a0375c5f2459a62fb`
and frontend
`101 files/1177/90f56093290c70a27369296ec8d8c7de99d084a091134994ae6451bc8e45743b`.
The dynamic route census reproduced
`175/488231c63e8c9bb0a28a6baf5e972c959c7eeddf9cc5fa10cdffc3330bc95aea`;
171 product route rows also matched an independent static AST projection with
zero unexplained row in either direction and zero network attempt.

The independent candidate scanner regenerated all
`10446/d1ab1f1a1c7001799bde2dcedcc2e4424af670e1ec1f2f38737f8a5fb671f8e9`
candidate rows byte-for-byte. The independent projector then rebuilt and
compared all 23 tracked authority payloads, including adjudications
`10446/75c39f535b5ec9f72476e98d61cc664a95d4a1d9adc23eca787d242838ba7041`,
surfaces
`478/5ad4e381d9eae04f9e1c060977290039337d853456e25079e84afd400fda2379`,
focused nodes
`1897/57c1b5145529ae0b36f4068406c67d40a7610f5cd0b3472ec0bd9e88dfeefbf9`,
all ten classification projections, all five disposition partitions, and all
four no-tail path sets. Every rebuilt file is byte-identical to its tracked
authority. The tracked inventory manifest remains
`dbc0a2ea96ca3867fe047a4fc8bbd6847f8cb1027caea32c17716d1c9b3d9e0c`.

The encrypted-path witness again proved the three frozen/main blob identities
equal before admitting a zero-row plaintext-claim result. No plaintext was
copied. Sixteen directly pinned Docker/SQL source blobs are byte-identical
between the frozen source and Task 3 tip. The complete archive/history
dispositions and future delete ownership remain covered by the independently
rebuilt canonical authorities and path sets.

### Runtime, safety, and rejected reviewer iterations

Both detached worktrees used worktree-local no-op git-crypt filters, empty real
`data/` directories, no `config/.env`, and only the reviewed root
`node_modules` link. They ended clean. The private config witness remains
exactly `{"database_url_key_present":true,"mode":"600","size":6463,"tracked":false}`;
no right-hand-side value or file content entered an artifact. Reviewer
processes after admission, provider requests, and route-census network attempts
are all zero.

Process census also recorded one pre-existing main-tree `python -m src.api`
sidecar, PID `3718636`, holding `market_data.db` and its WAL/SHM files. It
predated Task 4, was not started, contacted, stopped, or used by admission, and
is explicitly separated from the zero reviewer-worktree process/open boundary.

Rejected reviewer-tool iterations are retained as bounded notes: route-event
shape, requirements comments, encrypted-path exclusion, CLI AST anchoring,
standalone/identifier text overlap, path-scoped CLI handoff, add-path existence,
worktree-local git-crypt status, non-destructive packet assembly, and a first
leak audit that rejected its own machine path plus generated bytecode. None
changed source, product, test, dependency, config, database, provider, or
canonical authority bytes.

### Final packet and gate

Packet
`/tmp/pg-runtime-inventory-task4-7afbbf83/admission-packet-final` contains 97
verified payloads. `SHA256SUMS` has SHA-256
`c3f6cbf5c6dae3b0bc046bc28c4bbc64c836d51172630c4b59b524a5ac255021`.
The final leak audit scanned 96 payload inputs and reports zero violation; its
15 admitted pattern hits are exactly the three public redaction-test fixture
IDs repeated in backend collection/projection formats. Main and implementation
trees were clean before this status-only commit, the implementation range is
docs-only from `729d8514`, and no authority row changed during admission.
